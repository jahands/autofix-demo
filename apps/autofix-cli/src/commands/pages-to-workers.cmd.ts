...
import { Framework, ProjectDetector } from '../core/project.detection.js'
...
export const pagesToWorkersCmd = new Command('pages-to-workers')
	...
	.action(async (options) => {
		...
		try {
			...
			const result = await handler.migrate(
				projectPath,
				validatedOptions.pagesBuildCommand,
				validatedOptions.dryRun
			)
			...
			if (result.success) {
				...
				const errorOutput = formatter.formatError('migration_failed', 'Migration failed', {
			...
			const errorOutput = formatter.formatError('unexpected_error', error.message, {
